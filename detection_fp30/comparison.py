...
def images_to_video(image_dir, output_path, fps=1, type='ref'):
    if type == 'ref':
        img_files = sorted([f for f in os.listdir(image_dir) if f.startswith('ref') ], key=natural_sort_key)
    else:
        img_files = sorted([f for f in os.listdir(image_dir) if f.startswith('target') ],key=natural_sort_key)
...
def main():
    ...
    images_to_video(frames_dir, ref_frames_video, fps=30, type='ref')
    ...
    images_to_video(frames_dir, test_frames_video, fps=30, type='align')
